...
.advisory-box {
    ...
    padding:8px 10px; line-height:1.65; max-height:22vh; overflow-y:auto;
    width: 100%;
}
...
DISEASE_CLASSES  = ['Anthracnose', 'Styler-End-Rot', 'Scab', 'Healthy']
...
def generate_pdf_report(
    ...
    methodology = [
        ...
        ("Advisory", "LangGraph multi-agent pipeline using Llama3 + Qwen2.5 response comparison, "
         "judge selection, and fallback templates."),
    ]
...
with left:
    ...
    if file:
        ...
        # ── Compute predictions (needed here for advisory section below) ──
        with torch.no_grad():
        ...
        # ── Store in session_state so other panels can access ──
...
with center:
    if file:
        # Retrieve from session_state (computed in left column above)
        m_cls  = st.session_state["m_cls"]
        d_cls  = st.session_state["d_cls"]
        m_conf = st.session_state["m_conf"]
        d_conf = st.session_state["d_conf"]
        m_probs = st.session_state["m_probs"]
        d_probs = st.session_state["d_probs"]
    ...
    # ════════════════════════════════════════
    # AI ADVISORY — CENTER (below XAI)
    # ════════════════════════════════════════

    st.markdown(f"""
    <div class="adv-header" style="margin-top:14px;">
        <p class="slbl" style="margin:0;">🤖 AI Advisory</p>
        {_mode_pill_html(st.session_state.get("graph_mode", "normal"))}
    </div>
    """, unsafe_allow_html=True)

    col_btn, col_text = st.columns([0.8, 2.2])

    #BUTTON
    with col_btn:
        if st.button("Generate Expert Advice", use_container_width=True):
            with st.spinner("Running Multi-LLM Agentic AI pipeline..."):
            ...
            engine_text = "Multi-LLM Agentic AI" if used_llm else "Smart Fallback Engine"
            st.caption(f"Engine Used: {engine_text}")

    # TEXT OUTPUT (SIDE OF BUTTON)
    with col_text:
        if "advice" in st.session_state:
            advice = st.session_state["advice"]

            if st.session_state.get("graph_mode") == "uncertain":
                st.markdown("""
...
with right:
    if file:
        ...
        # ════════════════════════════════════════
        # PDF DOWNLOAD — bottom of right column
        # ════════════════════════════════════════
        st.markdown('<p class="slbl" style="margin-top:18px;">📄 Report</p>',
                    unsafe_allow_html=True)
