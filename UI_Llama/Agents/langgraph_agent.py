...
Uses Llama (via Ollama) for local LLM inference — no API key required.
...
Module dependencies (refactored):
    ...
    - models.py        → AdvisoryState, domain constants, Llama LLM interface
"""
...
# This file is loaded via importlib by Advisory.py, so standard package-relative
# imports (from Agents.X) may not resolve.  We add the parent of this file's
# directory to sys.path so that "import Agents.X" always works.
...
def decision_router_node(state: AdvisoryState) -> AdvisoryState:
    if state["disease"] in ("Healthy", "healthy"):
        state["mode"] = "harvest"
    else:
...
def triage_node(state: AdvisoryState) -> AdvisoryState:
    ...
    if sev == "high" and mat == "Mature":
        ...
        urgency = "High severity disease on non-mature fruit — fungicide is the top priority."
    elif sev == "moderate":
...
# ═══════════════════════════════════════════════════════════════
# LLM NODES  (now using Llama via Ollama)
# ═══════════════════════════════════════════════════════════════

def harvest_node(state: AdvisoryState) -> AdvisoryState:
    """Harvest-focused advisory for healthy fruit."""
    mat    = state["maturity"]
    hw     = state["harvest_window"]
    m_tier = _conf_tier(state["m_conf"])
    d_tier = _conf_tier(state["d_conf"])

    prompt = build_harvest_prompt(mat, hw, m_tier, d_tier)
    try:
        state["raw_llm_response"] = _call_llama(_SYSTEM_HARVEST, prompt, max_output_tokens=1500)
        state["used_llm"] = True
    except Exception:
        traceback.print_exc()
        state["raw_llm_response"] = ""
...
def treatment_node(state: AdvisoryState) -> AdvisoryState:
    ...
    sev   = state["severity"]
    hw    = state["harvest_window"]
    notes = state["triage_notes"]
    m_tier = _conf_tier(state["m_conf"])
    d_tier = _conf_tier(state["d_conf"])

    prompt = build_treatment_prompt(mat, dis, sev, hw, notes, m_tier, d_tier)
    try:
        state["raw_llm_response"] = _call_llama(_SYSTEM_TREATMENT, prompt, max_output_tokens=1800)
        state["used_llm"] = True
    except Exception:
        traceback.print_exc()
        state["raw_llm_response"] = ""
...
def uncertainty_node(state: AdvisoryState) -> AdvisoryState:
    ...
    prompt = build_uncertainty_prompt(mat, dis, m_tier, d_tier)
    try:
        state["raw_llm_response"] = _call_llama(_SYSTEM_UNCERTAINTY, prompt, max_output_tokens=1200)
        state["used_llm"] = True
    except Exception:
        traceback.print_exc()
        state["raw_llm_response"] = ""
...
def formatter_node(state: AdvisoryState) -> AdvisoryState:
    if state["used_llm"] and state["raw_llm_response"]:
        ...
        key = (state["maturity"], state["disease"])
        fallback = RULE_BASED_TEMPLATES.get(
            key,
            f"Advisory unavailable for {state['maturity']} / {state['disease']}. "
            "Please consult a local agricultural extension officer."
        )
        state["response"] = (
            fallback + "\n\n"
            f"[Model confidence — Maturity: {state['m_conf']*100:.1f}%  "
            f"Disease: {state['d_conf']*100:.1f}%]"
        )
        state["used_llm"] = False
    return state
...
def generate_advisory(
    ...
    disease  : str   — e.g. "Healthy", "Anthracnose", "Scab", "Styler Rot"
    ...
    Requirements
    ------------
    Ollama must be running locally (ollama serve) with a Llama model pulled.
    """
    ...
    try:
        ...
        traceback.print_exc()
        key = (maturity.strip().title(), disease.strip().title())
        advice = RULE_BASED_TEMPLATES.get(
            key,
            f"Advisory service temporarily unavailable for {maturity} / {disease}."
        )
        return advice, False
